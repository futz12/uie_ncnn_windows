...
UIEModel::UIEModel(const std::string& model_file,
    ...
    model.load_param(params_file.c_str());
...
void UIEModel::Preprocess(
    ...
    std::vector< std::vector<ncnn::Mat>> &inputs) {
  // 1. Tokenize the short texts and short prompts
    encodings.clear();
    inputs.clear();

    encodings.resize(input_texts.size());
    inputs.resize(input_texts.size());

    // mat {ids,typeids,posids,mask}
  for (int i = 0; i < input_texts.size(); ++i) {
      fast_tokenizer::core::EncodeInput text_pair_input = std::pair<std::string, std::string>(prompts[i], input_texts[i]);
      tokenizer_.EncodePairStrings(text_pair_input, &encodings[i]);

      inputs[i].resize(4);
      inputs[i][0] = ncnn::Mat(encodings[i].GetIds().size());
      inputs[i][1] = ncnn::Mat(encodings[i].GetTypeIds().size());
      inputs[i][2] = ncnn::Mat(encodings[i].GetIds().size());
      inputs[i][3] = ncnn::Mat(encodings[i].GetAttentionMask().size());

      // 2.2 Set the value of data
      int32_t* input_ids_ptr =
          reinterpret_cast<int32_t*>(inputs[i][0].data);
      int32_t* type_ids_ptr =
          reinterpret_cast<int32_t*>(inputs[i][1].data);
      int32_t* pos_ids_ptr = reinterpret_cast<int32_t*>(inputs[i][2].data);
      int32_t* attn_mask_ptr =
          reinterpret_cast<int32_t*>(inputs[i][3].data);
      ...
      std::copy(curr_input_ids.begin(), curr_input_ids.end(), input_ids_ptr);
      std::copy(curr_type_ids.begin(), curr_type_ids.end(), type_ids_ptr);
      std::iota(pos_ids_ptr, pos_ids_ptr + encodings[i].GetIds().size(), 0);
      std::copy(curr_attn_mask.begin(), curr_attn_mask.end(), attn_mask_ptr);
  }

}

void UIEModel::Postprocess(
    const std::vector<ncnn::Mat>& output0,
    const std::vector<ncnn::Mat>& output1,
  ...
  GetCandidateIdx(output0,
                  &start_candidate_idx_prob, position_prob_);
  GetCandidateIdx(output1,
...
void UIEModel::Predict(
  ...
  while (!nodes.empty()) {
    ...
    if (has_prompt) {
      ...
      // 3. Infer
      std::vector<ncnn::Mat> output0, output1;

      for (int i = 0; i < inputs.size(); ++i) {
        // mat {ids,typeids,posids,mask}
        // input {ids,posids,typeids,mask}
        auto ex = model.create_extractor();
        ex.input("in0", inputs[i][0]);
        ex.input("in1", inputs[i][2]);
        ex.input("in2", inputs[i][1]);
        ex.input("in3", inputs[i][3]);

        int failed = 0;

        ncnn::Mat _out0, _out1;

        failed |= ex.extract("out0", _out0);
        failed |= ex.extract("out1", _out1);

        if (failed) {
          std::cerr << "Failed to inference while using model:" << ModelName()
                  << "." << std::endl;
        }

        output0.push_back(_out0);
        output1.push_back(_out1);
      }
      // 4. Convert FDTensor to UIEResult
      Postprocess(output0, output1, encodings, short_input_texts, short_prompts,
